...
abstract class TBase {
  static $allowEnumConversion = false; // not using it : gaurav
  static $convertToArrays = true; 
  ...
  private function _readEnum(&$var, $spec, $input) {
	...
	$namesArrs = $className->getStaticPropertyValue('__names');
	//print_r($namesArrs);
        if(isset($namesArrs[$tmp])) $var = $namesArrs[$tmp];
        else $var = $tmp;
      } else $var = $tmp;
    }
    else $var = $tmp;
  ...
  private function _readStruct(&$var, $fspec, $input) {
    ...
    if(self::$convertToArrays) {
      ...
      foreach($refObj->getStaticPropertyValue('_TSPEC') as $field) {
        $f = $field['var'];
        if(isset($field['enum'])){
            //print_r($field['enum']);
            $enumClassName = str_replace('E_', '', $field['enum']);
            $eclass = new $enumClassName;
            $rval = $obj->$f;
            //$var[$f] =  $eclass::$__names[$rval]; 
	    $refEnumClass = new ReflectionClass($eclass);
            $namesArrs = $refEnumClass->getStaticPropertyValue('__names');    
            $var->$f =  $namesArrs[$rval];     
        }
        else{
